...
class JsonWrapper internal constructor(private val json: JSON) {
    ...
    fun getStrOrNull(path: String): String? = json.getByPath(path) as String?
    
    fun getIntOrNull(path: String): Int? = json.getByPath(path) as Int?
    
    fun getLongOrNull(path: String): Long? = json.getByPath(path) as Long?
    
    fun getBoolOrNull(path: String): Boolean? = json.getByPath(path) as Boolean?
    
    fun getDoubleOrNull(path: String): Double? = json.getByPath(path) as Double?
}